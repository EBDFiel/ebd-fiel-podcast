...
export async function POST(request: Request) {
  try {
    ...
    const duration = Math.min(15, Math.max(5, Number(form.get("duration")) || 10));
    const parts: any[] = [];
    if (mode === "pdf") {
    ...
    const targetWords = duration * 125;
...
- A conversa tem exatamente dois participantes identificados como "Apresentador:" e "Comentarista:".
- O Apresentador acolhe, apresenta o assunto e faz perguntas naturais.
- O Comentarista explica biblicamente com clareza e aplicações concretas.
- Inclua abertura curta: "Olá, seja bem-vindo ao EBD Fiel Podcast".
...
{"title":"título final","description":"descrição de até 240 caracteres","script":"Apresentador: ...\\n\\nComentarista: ..."}` });
