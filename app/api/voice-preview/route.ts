...
export async function POST(request: Request) {
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) { response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-tts-preview:generateContent?key=${key}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: voice } } } } }) }); data = await response.json(); if (response.ok) break; if (response.status === 429 && attempt === 0) { const seconds = Number(String(data?.error?.message || "").match(/retry in ([0-9.]+)s/i)?.[1] || 60); await new Promise(resolve => setTimeout(resolve, Math.min(70000, Math.max(58000, (seconds + 3) * 1000)))); } }
    if (!response?.ok) return Response.json({ error: data?.error?.message || "Não foi possível gerar a amostra." }, { status: response?.status || 502 }); const encoded = data?.candidates?.[0]?.content?.parts?.find((item: any) => item.inlineData?.data)?.inlineData?.data; if (!encoded) return Response.json({ error: "A API não retornou a amostra." }, { status: 502 });
